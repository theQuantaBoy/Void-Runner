...
int handle_location(int level_num)
{
    ...
    else if (enemy_index == -1 && object_index != -1)
    {
        ...
        case (Coin):
        {
            if (level[level_num].room[hero.last_room].type != Nightmare)
            {
...
void show_current_weapon()
{
    ...
    // Include the currently equipped weapon in the count
...
int run_game_level(int i)
{
    ...
    while (1)
    {
        ...
        // if (hero.health <= 0)
        // {
        //     print_message("You have been defeated...", "Game Over");
        //     break;
        // }
...
// Mace attack (damages all adjacent enemies)
...
// Dagger attack (throws dagger in a direction, damages first enemy hit)
...
// Magic Wand attack (throws wand in a direction, damages and stuns enemies)
...
// Normal Arrow attack (throws arrow in a direction, damages first enemy hit)
    ...
    { // Arrow range is 5 units
        ...
        for (int j = 0; j < level[level_num].enemy_num; j++)
        {
            ...
            if (e->location.y == arrow_pos.y && e->location.x == arrow_pos.x && e->health > 0)
            {
                ...
                if (got_input == 0)
                    result = getch();
                return result; // Arrow stops after hitting an enemy
            }
        }
    }
...
// Gun attack (damages all enemies in a straight line)
