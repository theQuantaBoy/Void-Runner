...
void show_weapon_inventory()
{
    ...
    while (1)
    {
        ...
        mvprintw(LINES - 5, 5, "Weapon Inventory (Press '2' to close menu.)");

        if (choice == 0)
            attron(A_REVERSE);
        mvprintw(LINES - 3, 8, "Mace (🪓): %d", mace);
        if (choice == 0)
            attroff(A_REVERSE);

        if (choice == 1)
            attron(A_REVERSE);
        mvprintw(LINES - 3, 8 + (COLS / 6), "Dagger (🔪): %d", dagger);
        if (choice == 1)
            attroff(A_REVERSE);

        if (choice == 2)
            attron(A_REVERSE);
        mvprintw(LINES - 3, 8 + ((2 * (COLS / 6))), "Magic Wand (✨): %d", wand);
        if (choice == 2)
            attroff(A_REVERSE);

        if (choice == 3)
            attron(A_REVERSE);
        mvprintw(LINES - 3, 8 + ((3 * (COLS / 6))), "Normal Arrow (🏹): %d", arrow);
        if (choice == 3)
            attroff(A_REVERSE);

        if (choice == 4)
            attron(A_REVERSE);
        mvprintw(LINES - 3, 8 + ((4 * (COLS / 6))), "Gun (🔫): %d", sword);
        if (choice == 4)
            attroff(A_REVERSE);
    ...
    void remove_weapom_from_inventory(int weapon_index)
    {
    ...
    void run_game_level(int i)
    {
        ...
        while (1)
        {
            ...
            else if (location_result == 20) // deamon
            {
                print_message("The deamon is hurting you.", "Press SPACE");
                input = getch();
                if (input == ' ')
                {
                    move(LINES - 5, 0);
    ...
    void play_playlist(const char **playlist)
    {
        ...
        {
            current_track = 0; // Loop back to the start of the playlist
        }
        ...
        Mix_PlayMusic(music, 0); // Play the music once (0 means no loop)
        current_track++;
    }
